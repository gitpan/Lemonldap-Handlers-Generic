...
if(defined($config->{ENCRYPTIONKEY})){
	my $clef = $config->{ENCRYPTIONKEY};
        my $cipher = new Crypt::CBC(-key => $clef,-cipher => 'Blowfish',-iv => 'lemonlda',-header =>'none');
...
sub goPortal {
	...
	$urlc_init.="?".$r->args if $r->args; 
	 ...
	 $log->warn("$CONFIG{HANDLERID}: ERROR OF LOCKING  ON :$id") if ($op eq 't') ; 
        my $messagelog =  "$CONFIG{HANDLERID} : Redirect to portal (url was ".$urlc_init.")";
        $log->info($messagelog); 
	return REDIRECT;
